...
// Seeds or tops up the lottery prize pot with FLOW treated as yield.
...
transaction(poolId: String, amount: UFix64) {
    prepare(user: auth(Storage) &Account) {
        ...
        let yieldVault <- flowVault.withdraw(amount: amount) as! @FlowToken.Vault
        pool.accumulateYield(from: <- yieldVault)
    }
}